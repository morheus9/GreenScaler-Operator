...
func main() {
	...
	// If --enable-http2 is false (default), disable HTTP/2 on metrics/webhook servers
	// to reduce exposure to HTTP/2 stream cancellation and rapid reset issues. See:
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "d9a2dc99.example.com",
	...
	if err := (&controller.GreenScalerServiceReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "GreenScalerService")
		os.Exit(1)
	}
